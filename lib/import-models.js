...
export default async function ({ apiKey, models, cleanup = false }) {
  ...
  if (cleanup) {
    for (const existingField of existingFields) {
      const updatedField = updatedFields.find((m) => m.id === existingField.id && m.itemType === existingField.itemType);
      if (!updatedField) {
    ...
    for (const existingItemType of existingItemTypes) {
      ...
      if (!updatedItemType) {
        console.log(`itemType ${existingItemType.name} should be removed, deleting...`);
