...
export default async function ({ apiKey, models, cleanup = false }) {
  ...
  for (const itemType of itemTypes) {
    ...
    if (newTitleField || newOrderingField) {
      console.log(`itemType ${existingItemType.apiKey} updating ${newTitleField && `titleField (${newTitleField})`}${newOrderingField && ` & orderingField (${newOrderingField})`}`);
  ...
  for (const field of fields) {
    ...
    if (field.position !== existingField.position) {
